...
?>
  ...
  <div class="row">
  <p></p>
  <h4 align="center"><b>Laporan Kas per 
  <?php if($bln==""){
    echo $thn;
  } elseif ($thn==""){
    echo "Bulan ".$bln;
  ...
  <table class="table table-bordered table-condensed table-responsive table-striped" id="export" align="center">
    ...
    <?php
        ...
        if($thn=="" && $bln==""){
            ...
            $sql="SELECT *,DATE_FORMAT(tgl, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vkas WHERE Tahun='$thn'";
        }elseif($thn==""){
            $sql="SELECT *,DATE_FORMAT(tgl, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vkas WHERE Bulan='$bln'";
        } else{
            $sql="SELECT *,DATE_FORMAT(tgl, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vkas WHERE Bulan='$bln' AND Tahun='$thn'";
        ...
        <td><?php echo "Kas ".$data['jenis']; ?></td>
        <?php
        if( $data['jenis']=="Masuk"){?>
          <td><?php echo number_format($data['jumlah']); ?></td>
          <?php
        } else{?>
          <td style="color:red;"><?php echo number_format($data['keluar']); ?></td>
          <?php
        }?>
        </tr>
        <?php  
        ...
        ?>
        <tr>
          <td colspan="3" align="right"><b>Total Pemasukan</b></td>
          <td colspan="2"><b>Rp. <?php echo number_format(@$hitung);?></b></td>
        </tr>
        <tr>
          <td colspan="3" align="right" style="color:red;"><b>Total Pengeluaran</b></td>
          <td colspan="2" style="color:red;"><b>Rp. <?php echo number_format(@$hitung1);?></b></td>
        </tr>
        <tr>
          <td colspan="3" align="right"><b>Saldo</b></td>
          <td colspan="2"><b>Rp. <?php echo number_format(@$keseluruhan);?></b></td>
        </tr>
