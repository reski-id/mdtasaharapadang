 ...
 <span class="glyphicon glyphicon-print"> Print</span> </button>
  ...
  <table class="table table-bordered table-condensed table-responsive table-striped" id="export" align="center">
    ...
    <?php
        ...
        if($kate=="" && $bln=="" && $proses==""){
          echo "<script type='text/javascript'> alert('Silahkan Pilih minimal Salah satu tanggal/nama/Jenis Tunggakan..'); window.location.href='laporantungakan_2.php';
          </script>"; 
      } elseif($kate=="" && $proses==""){ //ktg dan proses kosong
          $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE bulan='$bln'";
      } elseif($bln=="" && $kate==""){ //bln dan kate kosong
          $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE proses='$proses'";
      } elseif($bln=="" && $kate==""){ //bln dan proses kosong
          $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE nama='$kate'";
      } elseif($proses==""){ //proses kosong
          $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE bulan='$bln' AND nama='$kate'";
      } elseif($kate==""){ //kate kosong
        $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE bulan='$bln' AND proses='$proses'";
      } elseif($bln==""){ //bln kosong
        $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE nama='$kate' AND proses='$proses'";
      } else{ //tiga2  berisi
          $sql="SELECT *,DATE_FORMAT(tanggal, '%d %M %Y, %k:%i:%s') AS 'tanggal' FROM vtb_tunggakan WHERE bulan='$bln' AND nama='$kate' AND proses='$proses'";
