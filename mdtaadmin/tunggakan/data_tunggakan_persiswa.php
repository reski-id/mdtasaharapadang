...
?>
  ...
  <table class="table table-condensed table-bordered" id="table2">
    ...
    <?php
        include '../lib/koneksi.php';
        $sql = "SELECT t.idtunggakan,t.tanggal,s.NIS,s.NamaSiswa,t.jumlah,t.kettungakan,t.proses,t.jenis_pembayaran FROM tb_siswa s JOIN tb_tunggakan t ON s.NIS=t.NIS WHERE t.proses='Y' ORDER BY t.tanggal DESC";
        $result = mysqli_query($conn, $sql);
        $i = 1;
        $rows = mysqli_num_rows($result);
        if ($rows == 0) {
